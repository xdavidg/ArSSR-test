...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-lr", type=float, default=1e-4, dest="lr", help="the initial learning rate"
    )
    parser.add_argument(
        "-lr_decay_epoch",
        type=int,
    ...
    val_loader = data.loader_train(
        in_path_hr=hr_data_val, batch_size=1, sample_size=sample_size, is_train=False
    )
    ...
    # -----------------------
    DEVICE = torch.device(
        "cuda:{}".format(str(gpu) if torch.cuda.is_available() else "cpu")
    )
    ArSSR = model.ArSSR(
    ...
    for e in range(epoch):
        ...
        with torch.no_grad():
            ...
            for i, (img_lr, xyz_hr, img_hr) in enumerate(val_loader):
                img_lr = img_lr.unsqueeze(1).to(DEVICE).float()  # N×1×h×w×d
                xyz_hr = xyz_hr.view(
                    1, -1, 3).to(DEVICE).float()  # N×Q×3 (Q=H×W×D)
                H, W, D = img_hr.shape[-3:]
                img_hr = (
                    img_hr.to(DEVICE).float().view(1, -1).unsqueeze(-1)
                )  # N×Q×1 (Q=H×W×D)
                ...
                if (e + 1) % summary_epoch == 0:
                    # save model
                    torch.save(
                        ArSSR.state_dict(), "model/model_param_{}.pkl".format(e + 1)
                    )

        # Add validation metrics print statement
        print("Epoch [{}/{}], Train Loss: {:.6f}, Val Loss: {:.6f}, LR: {:.8f}".format(
            e + 1, epoch,
